...
cache_pos = torch.tensor([5], device=device, dtype=torch.int32)
dec_mask = causal_mask[5:6][None, None, :, :]

dec1 =  torch.tensor([[627]], device=device, dtype=torch.int32)
dec_pos1 = torch.tensor([5], device=device, dtype=torch.int32)
cache_pos1 = torch.tensor([6], device=device, dtype=torch.int32)
dec_mask1 = causal_mask[6:7][None, None, :, :]

dec2 =  torch.tensor([[627]], device=device, dtype=torch.int32)
dec_pos2 = torch.tensor([6], device=device, dtype=torch.int32)
cache_pos2 = torch.tensor([6], device=device, dtype=torch.int32)
dec_mask2 = causal_mask[7:8].clone()
dec_mask2[0][-1] = False
...
with torch.inference_mode():
        ...
        llm.gather_kv_incremental(indices=[6], offset=5)

        logits4 = llm.inference(input_ids=dec2, position_ids=dec_pos2, storage_ids=cache_pos2, attention_mask=dec_mask2).clone()
        #logits = model_forward(model=model,x=dec1,input_pos=dec_pos1, cache_pos=cache_pos1, attention_mask=dec_mask1)
        
del llm
llm = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-chat-hf", torch_dtype=torch.float32).to(device)
llm.eval()
